...
| Format | `just format` or `uv run ruff format src/blueprint_dryrun/` |
| Format check | `uv run ruff format --check .` |
| Typecheck | `uv run --extra web ty check src/blueprint_dryrun/` (ITM-026 / ADR-03; `--extra web` so web/ imports resolve) |
...
3. Init-system integrity (CI `blueprint-guard` + `init-integration` enforce
   ...
   identity value (`blueprint_dryrun`, `blueprint-dryrun`, `bpd`,
   `BPD`, author/owner names) must be listed in that value's `[[replace]]`
...
- Imports: sorted (ruff isort); absolute intra-package imports
  (`from blueprint_dryrun…` — the codebase convention)
...
(phrases like *"create a new project from blueprint-dryrun"*, *"start a
