...
- **Captured:** 2026-06-10
- **Scope:** init rebrand system (`init/`), CLI internals (`src/blueprint_dryrun/`)

## Problem

The init rebrand system replaces identity literals (`bpd`, `BPD`, …) per
...
match), so it never checks that a file containing `BPD` is listed under the
`app_name_upper` field specifically.

Result — a verified failure mode: a file listed under `app_name` (lowercase)
but not `app_name_upper` (uppercase) that contains a `BPD_*` literal passes
drift, yet a fork ships half-renamed (`widget` command still reading `BPD_*`).
This was hit twice during the CLI work (`_bpd_owned` in `core/logging.py`,
`bpd` in `tests/conftest.py`) and only caught by the slow full-rebrand
...
rename moves the *filename*, not the `bpd`/`BPD` occurrences inside the file
...
- `cli/main.py`: `_COMPLETE_VAR = "_BPD_COMPLETE"` → `f"_{APP_NAME.upper()}_COMPLETE"`
- `cli/main.py`: `auto_envvar_prefix: "BPD"` → `APP_NAME.upper()`
- `core/logging.py`: the `_bpd_owned` handler marker → `_OWNED_FLAG = f"_{APP_NAME}_owned"` + `setattr`/`getattr`

Leaves all **user-facing** env-var literals (`BPD_TOKEN`, `BPD_OUTPUT`, …)
explicit so `grep BPD_TOKEN src/` keeps working. `core/logging.py` drops out
...
2. Doesn't eliminate the manifest — docs, the spec, ADRs, CHANGELOG, and tests
   keep literal `bpd`/`BPD_*` (you can't derive prose; tests *should* assert
...
- Originated from the high-effort code review of the `bpd` CLI stack
