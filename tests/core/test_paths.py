...
def test_windows_config_uses_appdata(windows):
    ...
    assert paths.config_dir() == roaming / "bpd"
    assert paths.config_file() == roaming / "bpd" / "bpd_config.toml"


def test_windows_data_and_state_use_localappdata(windows):
    _roaming, local = windows
    assert paths.data_dir() == local / "bpd"
    assert paths.state_dir() == local / "bpd"


def test_windows_cache_nested_under_app_dir(windows):
    _roaming, local = windows
    assert paths.cache_dir() == local / "bpd" / "Cache"


def test_windows_xdg_override_still_wins(windows, monkeypatch, tmp_path):
    xdg = tmp_path / "xdg"
    monkeypatch.setenv("XDG_CONFIG_HOME", str(xdg))
    assert paths.config_dir() == xdg / "bpd"
    monkeypatch.setenv("XDG_CACHE_HOME", str(xdg))
    # explicit override keeps the POSIX <base>/bpd shape, no Cache nesting
    assert paths.cache_dir() == xdg / "bpd"
...
def test_posix_defaults_unchanged(monkeypatch):
    ...
    assert paths.cache_dir() == Path.home() / ".cache" / "bpd"
