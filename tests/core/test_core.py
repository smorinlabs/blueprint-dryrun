...
from blueprint_dryrun.core import (
    Config,
    ConfigPath,
    Project,
    ProjectList,
...
from blueprint_dryrun.core.config import (
    TOKEN_ENV_VAR,
    get_file_value,
    set_config_value,
)
from blueprint_dryrun.core.errors import ConfigError
from blueprint_dryrun.core.logging import LogFormat, configure_logging, get_logger
from blueprint_dryrun.core.settings import writable_keys
...
def test_token_never_read_from_file(tmp_path, monkeypatch):
    ...
    cfg_file = tmp_path / "bpd_config.toml"
...
def test_load_config_missing(monkeypatch):
    monkeypatch.delenv(TOKEN_ENV_VAR, raising=False)
    cfg = load_config(config_file="/nonexistent/path/bpd_config.toml")
...
def test_settings_defaults_when_no_file(monkeypatch):
    monkeypatch.delenv(TOKEN_ENV_VAR, raising=False)
    cfg = load_config(config_file="/nonexistent/bpd_config.toml")
...
def test_config_file_naming_under_xdg(tmp_path, monkeypatch):
    ...
    assert paths.config_file() == tmp_path / "bpd" / "bpd_config.toml"


def test_database_file_naming_under_xdg(tmp_path, monkeypatch):
    monkeypatch.setenv("XDG_DATA_HOME", str(tmp_path))
    assert paths.database_file() == tmp_path / "bpd" / "bpd_db.db"


def test_state_file_naming_under_xdg(tmp_path, monkeypatch):
    monkeypatch.setenv("XDG_STATE_HOME", str(tmp_path))
    assert paths.state_file("history") == tmp_path / "bpd" / "bpd_history.log"
...
def test_log_level_vocabularies_stay_in_sync():
    ...
    from blueprint_dryrun.core.logging import LOG_LEVELS
...
def test_output_format_vocabularies_stay_in_sync():
    ...
    from blueprint_dryrun.cli.output import OutputMode
    from blueprint_dryrun.core.settings import OutputSettings
...
def test_write_leaves_no_temp_files(tmp_path):
    ...
    set_config_value(cfg_file, "output.color", "never")
    set_config_value(cfg_file, "logging.level", "info")
    assert [p.name for p in tmp_path.iterdir()] == ["bpd_config.toml"]
    if sys.platform != "win32":  # POSIX modes don't exist on windows
        assert (cfg_file.stat().st_mode & 0o777) == 0o600
