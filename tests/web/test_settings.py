...
The literal BPD_WEB_* names below are intentional: they are registered in
...
from blueprint_dryrun.web.settings import ENV_PREFIX, WebSettings


def test_prefix_derives_from_app_name():
    assert ENV_PREFIX == "BPD_WEB_"
...
def test_env_overrides(monkeypatch):
    monkeypatch.setenv("BPD_WEB_PORT", "9001")
    monkeypatch.setenv("BPD_WEB_CORS_ORIGINS", '["https://app.example"]')
    monkeypatch.setenv("BPD_WEB_RATE_LIMIT", "100/minute")
    settings = WebSettings()
...
def test_invalid_value_fails_at_boot(monkeypatch):
    monkeypatch.setenv("BPD_WEB_PORT", "not-a-port")
