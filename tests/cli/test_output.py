...
from blueprint_dryrun.cli import output as output_mod
from blueprint_dryrun.cli.output import (
    OutputMode,
    Renderer,
    _resolve_pager_command,
)
from blueprint_dryrun.core.errors import ExitCode
from blueprint_dryrun.core.models import Project, ProjectList
...
def test_error_json_includes_error_code_and_hint(capsys):
    Renderer(OutputMode.JSON).error(
        ...
        error_code="BPD002",
        hint="set $BPD_TOKEN",
        traceback_path="/state/bpd_crash.log",
    )
    payload = json.loads(capsys.readouterr().err)
    assert payload["error"]["error_code"] == "BPD002"
    assert payload["error"]["hint"] == "set $BPD_TOKEN"
    assert payload["error"]["traceback_path"] == "/state/bpd_crash.log"
...
def test_error_text_shows_code_hint_and_traceback_path(capsys):
    ...
    assert "BPD002" in err
    assert "hint: set $BPD_TOKEN" in err
    assert "full traceback: /state/bpd_crash.log" in err
...
def test_pager_invoked_for_tall_terminal_output(monkeypatch):
    ...
    monkeypatch.setenv("BPD_PAGER", "fakepager --flag")
...
def test_pager_skipped_for_short_output(capsys, monkeypatch):
    ...
    monkeypatch.setenv("BPD_PAGER", "fakepager")
    monkeypatch.setattr(output_mod.subprocess, "run", explode)
    monkeypatch.setattr(output_mod, "_isatty", lambda console: True)
...
def test_pager_missing_binary_falls_back_to_plain_output(capsys, monkeypatch):
    ...
    monkeypatch.setenv("BPD_PAGER", "definitely-not-a-pager")
