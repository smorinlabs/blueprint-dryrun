# `bpd` web service — REST over the same core

The web service is the FastAPI counterpart of the `bpd` CLI: a thin adapter
...
python -m blueprint_dryrun.web    # production-shaped runner (settings-driven)
...
| `/readyz` | Readiness — same checks as `bpd doctor`; 503 problem doc on failure |
...
Everything is a `BPD_WEB_*` env var (prefix derived from the app name, so
forks rename cleanly). Invalid values fail at boot. Risky features are wired
but **off by default** — one env var enables each.

| Env var | Default | Effect |
|---------|---------|--------|
| `BPD_WEB_HOST` / `BPD_WEB_PORT` | `127.0.0.1` / `8000` | Bind address for the runner |
| `BPD_WEB_ROOT_PATH` | empty | Path prefix when behind a reverse proxy |
| `BPD_WEB_CORS_ORIGINS` | `[]` | JSON list; empty = CORS middleware not installed |
| `BPD_WEB_RATE_LIMIT` | unset | e.g. `100/minute`; unset = rate limiting off (WEB-22) |
| `BPD_WEB_METRICS_ENABLED` | `true` | `/metrics` endpoint (WEB-11) |
| `BPD_WEB_OTEL_ENABLED` | `false` | OTel tracing — needs the `otel` extra (WEB-10) |
| `BPD_WEB_IDEMPOTENCY_TTL_SECONDS` | `86400` | Idempotency replay-cache TTL (WEB-05) |
| `BPD_WEB_GRACEFUL_SHUTDOWN_SECONDS` | `10` | Drain window on shutdown (WEB-31) |

Secrets follow the CLI's rule: the API token resolves from `$BPD_TOKEN` only
...
```json
{
  ...
  "title": "Unauthorized",
  "status": 401,
  "detail": "no API token configured (set $BPD_TOKEN)",
...
```bash
# Liveness / readiness (readiness mirrors `bpd doctor`)
curl -s http://127.0.0.1:8000/healthz | jq
curl -s http://127.0.0.1:8000/readyz | jq

# Paginated projects (same models as `bpd projects list --json`)
