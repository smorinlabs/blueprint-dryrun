# `bpd` CLI Conventions — Requirements

- **Status:** Implemented (PRs #378–#380; naming per [ADR 0001](../adr/0001-app-short-name-bpd.md))
- **Type:** Design / requirements spec
- **Created:** 2026-06-09
- **Applies to:** the blueprint-dryrun CLI (`bpd`)
...
`blueprint-dryrun` CLI. App short name: **`bpd`**. Environment variable
prefix: **`BPD`**.
...
- **App name (namespace):** `bpd`
- **Env prefix:** `BPD` (every flag also resolves from `BPD_*`)
...
- **R1.2** Click is configured with `auto_envvar_prefix="BPD"` so every flag
  automatically resolves from a `BPD_*` environment variable. Env wiring is
  not hand-maintained.
...
- **R2.2** All **logs, diagnostics, and human messaging** are written to
  **stderr** only.
- **R2.3** The two streams are never mixed, so `bpd ... | jq` and similar
...
- **R4.3** Shell redirection (`bpd ... > file`) remains fully supported; the
...
- **R6.2** Config file path: `$XDG_CONFIG_HOME/bpd/bpd_config.toml`
  (default `~/.config/bpd/bpd_config.toml`). `$XDG_CONFIG_HOME` is honored
  when set (not hardcoded to `~/.config`).
- **R6.3** Optional layered discovery, each layer overriding the previous:
  1. system: `$XDG_CONFIG_DIRS/bpd/bpd_config.toml`
  2. user: `$XDG_CONFIG_HOME/bpd/bpd_config.toml`
  3. project-local: `./bpd_config.toml` (or `./.bpd_config.toml`)
- **R6.4** `--config PATH` (env `BPD_CONFIG`) overrides discovery entirely.
...
- **R8.2** The token is supplied via `--token` flag or the `BPD_TOKEN`
...
- **R10.4** `--log-level {debug,info,warning,error,critical}` (env
  `BPD_LOG_LEVEL`) is an explicit override of the console level.
...
- **R11.1** The file sink is enabled by `--log-file [PATH]` (env
  `BPD_LOG_FILE`) or by setting `logging.file` in config.
- **R11.2** When enabled without an explicit path, the default location is
  `$XDG_STATE_HOME/bpd/bpd.log` (default `~/.local/state/bpd/bpd.log`).
...
- **R11.4** The file sink has its own level (`logging.file_level`, env
  `BPD_LOG_FILE` companion / config), defaulting to `DEBUG`, independent of
  the console level.
- **R11.5** File sink format is controlled by `logging.format` (env
  `BPD_LOG_FORMAT`): `json` (JSONL, recommended for the file) or `text`.
...
- **R12.1** All env vars are prefixed `BPD_`. The full set:

| Variable          | Controls                                   | Equivalent flag        |
|-------------------|--------------------------------------------|------------------------|
| `BPD_CONFIG`     | Path to config file (overrides discovery)  | `--config`             |
| `BPD_OUTPUT`     | Output format (`text` \| `json`)           | `--output` / `-o`      |
| `BPD_COLOR`      | Color mode (`auto`\|`always`\|`never`)     | `--no-color`           |
| `BPD_LOG_LEVEL`  | Console log level                          | `--log-level`/`-v`/`-q`|
| `BPD_LOG_FILE`   | Log file path (presence enables file sink) | `--log-file`           |
| `BPD_LOG_FORMAT` | File sink format (`text` \| `json`)        | (config only)          |
| `BPD_TOKEN`      | Auth token (secret)                        | `--token`              |
...
## Appendix A — Example `bpd_config.toml`

```toml
# $XDG_CONFIG_HOME/bpd/bpd_config.toml

[output]
format = "text"        # text | json        (BPD_OUTPUT)
color  = "auto"        # auto | always | never

[logging]
level      = "warning" # console level       (BPD_LOG_LEVEL)
file       = ""        # empty = off; path enables file sink (BPD_LOG_FILE)
file_level = "debug"   # file sink level, independent of console
format     = "text"    # text | json (JSONL) for the file sink (BPD_LOG_FORMAT)
```
...
| Config file    | `$XDG_CONFIG_HOME/bpd/bpd_config.toml`            |
| Console logging| on, `WARNING`, to stderr                            |
| File logging   | off                                                 |
| File log path  | `$XDG_STATE_HOME/bpd/bpd.log` (when enabled)      |
| File log format| JSONL recommended                                   |
| Precedence     | flag → env → project → user → system → default      |

## Appendix D — Migration notes for `blueprint-dryrun`
...
5. Keep the token in an env var (current `PY_TOKEN` → `BPD_TOKEN`).
