...
[ADR 0005](https://github.com/smorinlabs/blueprint-dryrun/blob/main/docs/adr/0005-mise-flox-first-class-toolchains.md)):
...
Project requires Python 3.12+ (which is also specified inside [.python-version](https://github.com/smorinlabs/blueprint-dryrun/blob/main/.python-version) file)
...
# Format the code
uvx ruff format blueprint_dryrun/

# Run linter
uvx ruff check blueprint_dryrun/

# Run type checker
uv run ty check src/blueprint_dryrun/

# Run tests
uvx --with-editable . pytest

# Run tests with coverage
uvx --with pytest-cov --with-editable . pytest --cov=blueprint_dryrun --cov-report=term-missing
...
ruff format blueprint_dryrun/
ruff check blueprint_dryrun/
ty check src/blueprint_dryrun/
pytest --cov=blueprint_dryrun --cov-report=term-missing
...
1. **Project Name**:
   Replace `blueprint_dryrun` with your package name in the version verification step:
