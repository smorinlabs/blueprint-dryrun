...
2. **Configure ty**:
   ty works with sensible defaults out of the box; project-level options go in the `[tool.ty]` section of [`pyproject.toml`](https://github.com/smorinlabs/blueprint-dryrun/blob/main/pyproject.toml) if needed.
...
2. **Configure Pyright**:
   check the `[tool.pyright]` section of [`pyproject.toml`](https://github.com/smorinlabs/blueprint-dryrun/blob/main/pyproject.toml)
3. **Run Pyright**:
   To check your code with Pyright, run the following command:
   ```bash
   uvx --with-editable . pyright src/blueprint_dryrun/
   ```
