...
```sh
uvx ruff format blueprint_dryrun/       # Format code
uvx ruff check blueprint_dryrun/        # Run linter
uv run ty check src/blueprint_dryrun/  # Type check
uvx --with-editable . pytest               # Run tests
uvx --with pytest-cov --with-editable . pytest --cov=blueprint_dryrun --cov-report=term-missing  # Test coverage
```
