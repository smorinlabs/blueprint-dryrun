...
uv run python -c "import blueprint_dryrun; print(blueprint_dryrun.__version__)"
...
- **Build cannot find the package module**: check `[tool.uv.build-backend]` and keep `module-name = "blueprint_dryrun"` with `module-root = ""`.
