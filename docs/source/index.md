```{figure} _static/blueprint_dryrun_logo_100x100.png
:alt: blueprint-dryrun
:width: 100px
:align: left
```
...
![Changelog](https://img.shields.io/github/v/release/smorinlabs/blueprint-dryrun?include_prereleases&label=changelog)
![Tests](https://github.com/simonw/llm/workflows/Test/badge.svg)
![License](https://img.shields.io/badge/License-MIT-yellow.svg)
![GitHub stars](https://img.shields.io/github/stars/smorinlabs/blueprint-dryrun?style=social)
...
- [blueprint-dryrun Docs](https://blueprint-dryrun.readthedocs.io/en/latest/)
...
```bash
git clone https://github.com/smorinlabs/blueprint-dryrun
cd blueprint-dryrun
```
...
- **CLA (Contributor License Agreement) check `[TODO:LOOKUP ISSUE](https://github.com/smorinlabs/blueprint-dryrun/issues/162)`**: Ensure all contributors have signed appropriate licensing agreements, protecting the project legally.
...
After installing, the CLI is available as `bpd`:
...
`bpd` reads settings from a layered TOML config (system → user → project),
with per-setting precedence `flag → env (BPD_*) → project → user → system →
default`. Secrets are never stored in config — the API token resolves from
`--token` or `$BPD_TOKEN` only.
...
bpd config set output.format json
```
...
bpd projects list
bpd projects list --workspace "My Workspace"
bpd projects list --limit 50
bpd projects get 12345

# JSON output (pipe-safe; logs stay on stderr)
bpd projects list --json
...
[EXAMPLECLI.md](https://github.com/smorinlabs/blueprint-dryrun/blob/main/EXAMPLECLI.md).
