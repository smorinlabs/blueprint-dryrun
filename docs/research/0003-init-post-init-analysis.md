...
A thorough exploration of the `blueprint-dryrun` repository produced the
...
- **Package**: `src/blueprint_dryrun/` (PEP 420 layout with uv)
- **CLI**: `bpd` (Click-based, example in `src/blueprint_dryrun/cli/`)
...
[[replace]]
field = "package_name"
current = ["blueprint_dryrun"]
files = ["pyproject.toml", "src/blueprint_dryrun/__init__.py", ...]
...
4. **Apply** (remove → replace → rename):
   ...
   - **Rename**: Move files/dirs (e.g., `src/blueprint_dryrun/` → `src/{new_name}/`)
...
| 4 | **Fork** | `<user>/blueprint-dryrun` (name collides, owner differs) | Full blueprint history | Guard's skip-on-origin checks owner + name (must NOT skip forks; contributor sentinel is escape hatch) | L1 + L2 + contrib sentinel test |
...
BLUEPRINT_IDENTITY = {
    ...
    "app_name": "bpd",
    "app_name_upper": "BPD",  # derived, not prompted
    "author": "Steve Morin",
...
decisions about app_name (bpd), config handling, etc.
...
```
              ┌──────────────────────────────────────────────┐
              │       blueprint-dryrun (a working repo)   │
              └──────────────────────────────────────────────┘
                     │                            │
...
```
   ...
   (new doc mentions "bpd")           (doesn't know yet)
        │                                   │
        ▼                                   ▼
   ┌─────────────────────────────────────────────────────┐
