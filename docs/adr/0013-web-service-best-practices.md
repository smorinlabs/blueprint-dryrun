...
`blueprint_dryrun.core` (one data contract shared with the CLI), must not
...
6. **Production posture** — `python -m blueprint_dryrun.web` runs
...
- **Always-on OTel in the `web` extra** — rejected: the dependency tree is
  ...
  keeps `pip install blueprint-dryrun[web]` lean.
