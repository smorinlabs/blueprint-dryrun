...
py_package_name := "blueprint_dryrun"
...
[group('test'), group('dev')]
@coverage:
    uv run pytest --cov=blueprint_dryrun --cov-report=term-missing --cov-report=html --cov-report=xml
...
[group('build')]
@docker-web tag="bpd-web:dev": _guard
    docker build -t {{tag}} .
